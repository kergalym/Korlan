...
class PlayerDamages:
    ...
    def _do_damage(self, actor, hitbox_np, parent_np, enemy_npc_ref, request):
    ...
    def _trace_hit(self, node, actor, parent_np, request):
        # Get enemy based on its health and weapon state (equipped or not)
        if node is None and not node.get_name().endswith(":BS"):
            return None

        node_name = node.get_name().split(":")[0]
        if self.base.game_instance["actors_ref"].get(node_name) is None:
            return None

        enemy_npc_ref = self.base.game_instance["actors_ref"][node_name]
        if enemy_npc_ref is None:
            return None
        ...
        if enemy_npc_ref.get_python_tag("generic_states")['is_alive']:
            ...
            if hitbox_np:
                self._do_damage(actor, hitbox_np, parent_np, enemy_npc_ref, request)
            else:
    ...
    def player_hitbox_trace_task(self, actor, actor_rb_np, request, task):
        ...
        for node in parent_node.get_overlapping_nodes():
            ...
            self._trace_hit(node, actor, parent_np, request)
