...
if exists("GameData.mf"):
    ...
    if vfs.mount(mf, game_dir, VirtualFileSystem.MFReadOnly):
...
def check_and_do_cfg():
    ...
    if not exists("{0}/{1}".format(game_cfg_dir,
        ...
        try:
            ...
            force_do_cfg()
        if isdir(game_cfg_dir) is False:
            mkdir(game_cfg_dir)
            do_cfg()
            return True
        else:
...
def do_cfg():
    """ Function    : do_cfg
        ...
        Return      : None
    """
    if exists('{0}/{1}'.format(game_cfg_dir, game_settings_filename)) is False:
        if exists(game_cfg_dir) is False and isdir(game_cfg_dir) is False:
            mkdir(game_cfg_dir)
